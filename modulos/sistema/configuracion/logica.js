...
window._cfgToggleEye = function(inputId, btn) {
...
window.cfgCambiarPassword = async function() {
    ...
    if (!actual || !nueva) return showMsg('Completa todos los campos.', '#ef4444');
    if (nueva !== confirmar) return showMsg('Las contraseñas nuevas no coinciden.', '#ef4444');
    var correo = localStorage.getItem('fleet_correo') || '';
    try {
        var res = await fetch('/api/cambiar-password', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ correo: correo, passwordActual: actual, passwordNueva: nueva })
        });
        var json = await res.json();
        if (!res.ok) throw new Error(json.error || 'Error al cambiar contraseña');
        showMsg('Contraseña cambiada correctamente.', '#10b981');
        ['cfg-pass-actual', 'cfg-pass-nueva', 'cfg-pass-confirmar'].forEach(function(id) {
            var el = document.getElementById(id);
            if (el) el.value = '';
        });
    } catch(e) {
        showMsg(e.message, '#ef4444');
    }
};
