...
(function() {
    ...
    window.init_marsisa_combustible_vales = function() {
    ...
    window._cvCargarCatalogos = async function() {
        try {
            const res = await fetch('/api/combustible/catalogos?modulo=marsisa');
            const data = await res.json();
            if (data.ok) {
    ...
    window.cvEliminarSeleccionados = async function() {
        ...
        try {
            const res = await fetch('/api/combustible/vales/eliminar-masivo?hard=true&modulo=marsisa', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ids: Array.from(window._cvSeleccionados), modulo: 'marsisa' })
            });
            const data = await res.json();
            if (data.ok) {
    ...
    window.cvEliminarVale = async function(id) {
        ...
        try {
            const res = await fetch(`/api/combustible/vales/${id}?hard=true&modulo=marsisa`, { method: 'DELETE' });
            const data = await res.json();
            if (data.ok) {
    ...
    window.cvConfirmarImportacion = async function() {
        ...
        try {
            const res = await fetch('/api/combustible/vales/importar-masivo', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ vales: window._cvParsedImportData, modulo: 'marsisa' })
            });
            const data = await res.json();
    ...
    window.cvExportarExcel = async function() {
        ...
        const params = new URLSearchParams({ limit: 5000, modulo: 'marsisa' });
        const s = document.getElementById('cv-filter-search')?.value;
        const p = document.getElementById('cv-filter-placa')?.value;
        const c = document.getElementById('cv-filter-combustible')?.value;
        const e = document.getElementById('cv-filter-estado')?.value;
    ...
    window.cvGuardarFormulario = async function(event) {
        ...
        try {
            const url = isEdit ? `/api/combustible/vales/${id}?modulo=marsisa` : '/api/combustible/vales';
