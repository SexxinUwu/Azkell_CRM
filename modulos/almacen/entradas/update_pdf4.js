...
const startStr = 'window._entGenerarHtmlPDF = function(d) {';
const startIdx = code.indexOf(startStr);
const endStr = 'window.generarComprobanteEntrada = function(id) {';
const endIdx = code.indexOf(endStr);

if (startIdx !== -1 && endIdx !== -1) {
    code = code.substring(0, startIdx) + htmlBuilder + '\n\n' + code.substring(endIdx);
    fs.writeFileSync(path, code);
    console.log('Update 4 with subtitle removed applied successfully!');
} else {
    console.log('Could not find bounds to replace _entGenerarHtmlPDF');
}
