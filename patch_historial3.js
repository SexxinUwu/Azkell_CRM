...
const oldPart = "html += fld('Liberado por', esc(row.liberado_por || '?'));\n      html += '</div>';";
if (js.includes(oldPart)) {
    js = js.replace(oldPart, oldPart + "\n" + addOTsBox);
}

// Ensure the old code `??"` didn't ruin the match by trying an alternate
const oldPart2 = "html += fld('Liberado por', esc(row.liberado_por || '?\"'));\n      html += '</div>';";
if (js.includes(oldPart2)) {
    js = js.replace(oldPart2, oldPart2 + "\n" + addOTsBox);
}

// Or splitting by `html += fld('Liberado por'` just to be sure
let parts = js.split(/html \+= fld\('Liberado por'.*\);\s*html \+= '<\/div>';/);
if (parts.length === 2) {
    let match = js.match(/html \+= fld\('Liberado por'.*\);\s*html \+= '<\/div>';/)[0];
    js = parts[0] + match + "\n" + addOTsBox + parts[1];
}
