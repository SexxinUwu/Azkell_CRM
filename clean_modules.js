const fs = require('fs'); let p = 'modulos/sistema/usuarios/logica.js'; let c = fs.readFileSync(p, 'utf8'); let startIdx = c.indexOf('window._GU_MODULOS = window._GU_MODULOS || ['); let endIdx = c.indexOf('];', startIdx) + 2; let newMods = \window._GU_MODULOS = window._GU_MODULOS || [
    // FLOTA
    ...
    { grupo:'MANTENIMIENTO', key:'fleet',         nombre:'Fleetrun',         desc:'Datos de la flota y preventivos', lcad:true  },
...
];\; c = c.substring(0, startIdx) + newMods + c.substring(endIdx); fs.writeFileSync(p, c);
